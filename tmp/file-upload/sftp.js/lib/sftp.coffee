...
Path = require 'path'
KnownTextFileTypes = require './known_text_file_types'
    ...
    tmp.file (err, tmpFilePath, fd) =>
      ...
      this._runCommand "get #{@constructor.escape filePath} #{@constructor.escape tmpFilePath}", (data) =>
        ...
        if lines.length != 3
          ...
          lines.pop()
          callback new Error(lines.join "\n")
        else if this._isKnownTextFile(filePath)
          this._readTmpFile tmpFilePath, 'text', callback
        else
          childProcess.exec "file -b #{@constructor.escape tmpFilePath}", (err, fileType) =>
            if err
              ...
              this._readTmpFile tmpFilePath, fileType, callback

  _isKnownTextFile: (path) ->
    identifier = Path.extname(path).substr(1)
    # if the file has no extension,
    # try using the basename (eg. README)
    unless identifier.length
      identifier = Path.basename(path).toLowerCase()
    KnownTextFileTypes.indexOf(identifier) > -1

  _readTmpFile: (filePath, fileType, callback) ->
