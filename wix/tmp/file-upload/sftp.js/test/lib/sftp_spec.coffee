...
describe 'SFTP', ->
    ...
    context 'when temp file creation succeeds', ->
      ...
      context 'when _runCommand succeeds', ->
        beforeEach ->
          ...
          sinon.stub(sftp, '_isKnownTextFile')
          sinon.stub sftp, '_readTmpFile'
          sftp._runCommand.callsArgWith 1, output

        afterEach ->
          childProcess.exec.restore()
          sftp._isKnownTextFile.restore()
          sftp._readTmpFile.restore()

        it 'check whether the given file type is a text file', ->
          doAction()
          expect(sftp._isKnownTextFile).to.have.been.calledWith 'path/to/remote-file'

        context "when file is known to be a text file", ->
          beforeEach ->
            sftp._isKnownTextFile.returns true

          it 'reads the temp file and handles the callback', ->
            doAction()
            expect(sftp._readTmpFile).to.have.been.calledWith '/tmp/action/tempfile', 'text', cbSpy

        context "when file is not known to be a text file", ->
          beforeEach ->
            sftp._isKnownTextFile.returns false
          ...
          context 'when file command succeeds', ->
            ...
            it 'reads the temp file and handles the callback', ->
              doAction()
              expect(sftp._readTmpFile).to.have.been.calledWith '/tmp/action/tempfile', 'some file type', cbSpy

          context 'when file command fails with an error', ->
            error = null
  ...
  describe '#_isKnownTextFile', ->
    it 'returns true if path is a known text file', ->
      expect(sftp._isKnownTextFile('file.json')).to.be.true

    it 'returns true for known text files without extention', ->
      expect(sftp._isKnownTextFile('README')).to.be.true

    it 'returns true for known hidden text files', ->
      expect(sftp._isKnownTextFile('.gitignore')).to.be.true

    it 'returns false if path is not a known text file', ->
      expect(sftp._isKnownTextFile('file.tar')).to.be.false

  describe '#_readTmpFile', ->
    cbSpy = null
    ...
    doAction = ->
      sftp._readTmpFile '/tmp/file', 'text/plain', cbSpy

    context 'when readFile succeeds', ->
      buffer = new Buffer 'some file content'
      beforeEach ->
        fs.readFile.callsArgWith 1, null, buffer
        doAction()

      it 'unlinks the temp file', ->
        expect(fs.unlink).to.have.been.called

      it 'calls the callback with data and file type', ->
        expect(cbSpy).to.have.been.calledWith null, buffer, 'text/plain'
